"""python -m data.mcp                       serve the data layer over stdio
python -m data.mcp --http [HOST:]PORT     serve it over Streamable HTTP (/mcp, /health)
python -m data.mcp list                   list the tools
python -m data.mcp call NAME [JSON-ARGS]  run one tool and print its text"""

import json
import sys

from data.mcp import tools
from data.mcp.server import Server, ToolError, serve_http
