...
2. `NOT READY` names the problem. The usual fixes, in order: a stale bind
   ...
   `python orchestrator.py status` again; the database never became
...
`python orchestrator.py status` answers "is it up?" without touching
anything; `python orchestrator.py test` runs the suite inside the image.
