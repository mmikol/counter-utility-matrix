...
<!-- generated:catalog -->
243 files in `inference/strategies/`: 45 constraints (1 limits, 44 scored), 193 heuristics and 5 assumptions. Regenerated by `python -m db.mcp call db_docs`.
...
weight 1; when `team.availability < params.COINFLIP`; penalty `1.0`
params: COINFLIP=0.5
...
weight 1; when `team.cleanse < 1`; penalty `0.5`

A cleanse fits any composition because what it undoes, anti-heal, sleep, hack and burn, is somewhere on every red team's kit list. Kiriko is played in every kind of comp with every kind of tank for that reason. It charges half a point while the six has no pick with a cleanse.
...
`maximize team.aoe_count` - kit pieces tagged area of effect. weight 0.5; when `matchup.style_lean_red == 'brawl'`
...
`maximize team.aoe_count` - kit pieces tagged area of effect. weight 0.25; when `team.style_lean == 'brawl'`

A brawl six fights where both teams stand within arm's reach, and area damage hits everything in that press without aiming at any of it. Junkrat (5 pieces), Venture (4) and D.Mon, Lúcio, Baptiste and Wuyang (3 each) carry the most area damage among brawl picks; Reinhardt carries none tagged. Measured as kit pieces tagged area of effect, read only while brawl is the majority style.

##### Brawl swings at melee (`brawl-swings-melee`, damage)

`maximize team.melee` - picks with a melee weapon. weight 0.5; when `team.style_lean == 'brawl'`
...
`maximize team.burst_max` - the biggest single hit on the team, a headshot where one counts. weight 0.25; when `team.style_lean == 'dive'`
...
`maximize team.dmg_amp` - picks that amplify someone's damage. weight 0.25; when `team.style_lean == 'poke'`
...
`maximize team.hitscan` - picks with a hitscan weapon or ability. weight 0.75; when `team.style_lean == 'poke'`
...
`maximize team.burst_max` - the biggest single hit on the team, a headshot where one counts. weight 0.25; when `team.style_lean == 'poke'`
...
`maximize team.aoe_count` - kit pieces tagged area of effect. weight 0.25; when `enemy.barrier_count >= 2`
...
`maximize team.armor_total` - summed armor. weight 0.25
...
`maximize team.barrier_count` - picks with a barrier. weight 0.25; when `enemy.one_shots >= 1 and enemy.burst_max >= 300`
...
`maximize matchup.pool_diff` - blue effective HP minus red. weight 0.5; when `team.style_lean == 'brawl' and matchup.style_lean_red == 'brawl'`
...
`maximize team.pool_min` - the weakest pick's pool - focus fire finds the minimum. weight 0.75; when `team.style_lean == 'brawl'`
...
`maximize team.armor_total` - summed armor. weight 0.25; when `team.style_lean == 'brawl'`
...
`maximize team.invuln` - picks with an invulnerability. weight 0.75; when `enemy.burst_max >= 250`
...
`maximize team.armor_total` - summed armor. weight 0.75; when `team.tanks <= 1`
...
`maximize team.hitscan` - picks with a hitscan weapon or ability. weight 0.5; when `map.style_top == 'poke'`
...
`minimize team.deployables` - picks with deployables. weight 0.25; when `map.known == 1 and map.sided == 0`
...
`maximize matchup.ult_answers` - blue invulnerabilities plus cleanses. weight 0.75; when `matchup.ult_threat >= 1500`
...
`maximize team.banproof_coverage` - coverage recomputed without the highest-ban answerer. weight 0.25
...
`maximize team.barrier_hp` - summed barrier health the team fields. weight 0.5; when `matchup.ult_threat >= 1500`
...
`maximize team.beam` - picks with a beam. weight 0.75; when `matchup.style_lean_red == 'dive'`
...
`maximize team.cc_count` - picks with crowd control (stun, sleep, immobilize, hinder, knockback). weight 0.75; when `team.style_lean == 'brawl'`
...
`maximize team.cleanse` - picks with a cleanse. weight 0.5; when `enemy.cc_count >= 5`
...
`maximize team.answer_edges` - (enemy, pick) counter edges: picks answering enemies. weight 0.25; when `team.damage >= 3 and enemy.size >= 1`
...
`maximize team.invuln` - picks with an invulnerability. weight 0.5; when `matchup.style_lean_red == 'dive'`
...
`maximize team.range_median` - median of each pick's longest published range. weight 0.25; when `enemy.size >= 5 and enemy.range_max <= 40 and enemy.one_shots == 0`
...
`maximize matchup.ult_answers` - blue invulnerabilities plus cleanses. weight 0.5; when `enemy.size >= 1 and enemy.cooldown_median <= 7`
...
`minimize team.max_ban_rate` - the highest ban rate on the team. weight 0.25
...
`maximize team.availability` - chance every pick survives the ban screen: product of (1 - ban). weight 0.25; when `team.synergy_edges >= 1`
...
`maximize team.map_availability` - the same from this map's ban rates (the all-ranks ban where a map publishes none; equal to availability without a map). weight 0.5; when `map.known == 1`
...
`minimize team.range_max` - the longest range on the team. weight 0.75; when `team.style_lean == 'brawl'`
...
`maximize team.hps_floor` - summed sustained healing onto teammates, hp per second, reloads in. weight 0.25; when `team.style_lean == 'brawl'`
...
`maximize team.dmg_ults` - ultimates that carry a damage figure. weight 0.25; when `team.style_lean == 'brawl'`
...
`maximize team.mobility_count` - picks with a movement or evasive ability. weight 1.25; when `team.style_lean == 'dive'`
...
`maximize team.hps_floor` - summed sustained healing onto teammates, hp per second, reloads in. weight 0.75; when `team.style_lean == 'dive'`
...
`maximize team.flyers` - picks that fly or hover. weight 0.5; when `team.style_lean == 'dive'`
...
`maximize team.subrole_diversity` - distinct subroles / size (1.0 = every pick a different job). weight 0.5
...
`maximize team.mobility_count` - picks with a movement or evasive ability. weight 0.5; when `team.style_lean == 'poke'`
...
`maximize team.barrier_hp` - summed barrier health the team fields. weight 0.5; when `team.style_lean == 'poke'`
...
`maximize team.range_median` - median of each pick's longest published range. weight 1; when `team.style_lean == 'poke'`
...
`maximize team.range_min` - the shortest longest-range. weight 0.25; when `team.style_lean == 'poke'`
...
`maximize team.cc_count` - picks with crowd control (stun, sleep, immobilize, hinder, knockback). weight 0.75; when `team.tanks <= 1`
...
`minimize team.squish_count` - picks at or under 250 pool. weight 0.5; when `team.damage >= 3`
...
`maximize team.subrole_diversity` - distinct subroles / size (1.0 = every pick a different job). weight 0.25; when `team.damage >= 3`
...
`maximize team.dps_floor` - summed published per-second damage figures (a floor: misses and healing ignored). weight 0.25; when `team.supports >= 3`
...
`minimize team.exposed_count` - picks answered by at least one enemy. weight 0.25; when `team.tanks >= 2`
...
`maximize team.deployables` - picks with deployables. weight 1.25; when `map.side == 'defense'`
...
`maximize matchup.ult_answers` - blue invulnerabilities plus cleanses. weight 0.25; when `enemy.ult_cost_mean <= 2200 and enemy.size >= 3`
...
`maximize team.lifelines` - picks carrying any healing at all, their own included. weight 0.5
...
`maximize team.lifelines` - picks carrying any healing at all, their own included. weight 0.5; when `team.hps_ratio < 0.7`
...
`maximize matchup.ult_answers` - blue invulnerabilities plus cleanses. weight 0.5; when `enemy.dmg_ults >= 5`
...
`maximize team.heal_peak_total` - summed biggest single heal per pick, its own self-heal included. weight 0.5
...
`maximize team.lifelines` - picks carrying any healing at all, their own included. weight 0.5; when `matchup.style_lean_red == 'dive'`
...
`maximize team.hps_supports` - summed sustained healing across the supports, hp per second. weight 0.25; when `team.supports == 1`
...
`maximize team.mobility_count` - picks with a movement or evasive ability. weight 0.25; when `team.supports <= 1`
...
`maximize team.hps_supports` - summed sustained healing across the supports, hp per second. weight 0.25; when `team.tanks >= 2`
...
`minimize team.exposed_count` - picks answered by at least one enemy. weight 0.25; when `team.exposed_count >= 1`
...
`maximize team.core_size` - largest connected group in the team's synergy graph. weight 0.25; when `team.style_lean == 'dive'`
...
`minimize team.isolated_count` - picks with no authored partner on the team. weight 0.25; when `team.style_lean == 'dive'`
...
`maximize team.synergy_density` - synergy edges / possible pairs. weight 0.25; when `team.exposed_count >= 5`
...
`maximize team.synergy_score` - summed synergy scores among the picks. weight 0.25; when `enemy.size >= 1 and matchup.coverage_share < 0.5`
...
`maximize team.synergy_score` - summed synergy scores among the picks. weight 0.25; when `team.tanks >= 2`
...
`minimize team.cooldown_median` - median cooldown across every ability on the team. weight 0.5; when `team.style_lean == 'dive'`
...
`maximize team.invuln` - picks with an invulnerability. weight 0.25; when `enemy.size >= 3 and matchup.tempo_diff <= -2`
...
`minimize team.cooldown_median` - median cooldown across every ability on the team. weight 0.25
...
`maximize team.win_mean` - mean all-ranks win rate. weight 0.25; when `map.known == 1 and team.map_pick_mass < 40`
...
| `team.lifelines` | picks carrying any healing at all, their own included |
...
| `world.hps_bench` | 2 x the median sustained healing across the support roster |
