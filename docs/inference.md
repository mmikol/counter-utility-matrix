...
| `engine.py` | `infer` (the optimal six around the locked picks) and `evaluate` (a full six ranked against the field), each taking the board as a `Draft`, and `board`: at any stage of a draft, blue's optimal as the counter to red's selection, red's optimal as their counter to blue's, both current comps scored on those scales, blue's picks against red's best counter, blue's locked picks with the empty slots filled, red's likely starting comp, the fight odds, the game plan in prose, and the shapes the queue and the playbook's limits allow; a team past the queue's two tanks is refused. The current comp is private to `board`: it is scored under the bounds its seat's search froze. `board` runs one pass, its four searches split across the pool or each run in this process; a dead worker means that board runs again in this process and the pool is rebuilt. A `Brief` carries what a caller asks beyond the draft: the pool size, the weights, whether to solve the countered case (the MCP board does; the page's do not) and a check that a newer request superseded the board, which `Latest` hands a server per client; a superseded board stops at its next round, its unstarted tasks cancelled, and raises `Superseded`, a Refusal. |
| `result.py` | The records: `Result`, one seat's six with the picks and their reasons, `[F#]` citations into the board's FactSet, the score breakdown per strategy, the alternatives and the assumptions as "ground rules to reconcile against"; `Board`, the seven Results with the momentum and the plan; and the typed parts a payload carries (`Pick`, `Alternative`, `Consideration`, `Odds`, `Momentum`). Each renders as JSON (`to_dict`) and as text (`rendered`), and a result that cannot be a share of anything says why (`unscored`). |
| `plan.py` | The board in prose: `momentum`, the verdict read off the two current comps (each seat's share, a half-drafted seat's through its fill on both sides, the fight odds, blue against red's best counter), and `plan`, the game plan - the ground and its terrain, the style to play, what red's picks mean and which picks of the six answer them, the family to stay in, what the six is built for, and what it rests on - read off the same facts and strategies the solver scored. Where the playbook scores nothing the six is only the highest win rates the search found, and the plan says so: no counter to red and no fit claimed, and the style worded from the roles the six holds. |
| `parallel.py` | The process pool `board` splits its four searches across - blue's optimal, red's counter, the fill, the countered case - each in four rounds: the reference sample, the sample again for each hero's standing (its mean score across the reference sixes it is in, which ranks each role's pool of six), the enumeration, then the ranking and the local search. Only verdicts cross (hero ids, score, tie-break) and slices partition their round, so the answer does not depend on the split. The pool is `max(6, min(cores, 12))` spawned workers (`POOL`); `COUNTRIX_WORKERS` overrides; `COUNTRIX_PARALLEL=0`, a single core or a caller-supplied catalog keeps the board in one process. `warm` starts the workers at launch, and a worker that cannot start is written to stderr and leaves the board in one process. |
| `reach.py` | Every hero is the right pick somewhere: for a hero, a board that suits it (its maps, a red it answers, a side, the match's bans spent on the rivals holding its seat) on which it is in the optimal six, or the closest it came. The `reach` tool runs it; `tests/fixtures/reach.json` records a board per released hero beside the digest of the playbook that seated them, and the suite checks none is lost. `python -m scripts.reach` re-records it. A hero no board seats is one the facts or the strategies cannot see. |
| `tune.py` | `tune(id, field, value, reason)`: one frontmatter edit; `add(id, name, kind, prose, fields, reason)`: a new file from what the user gave and what `/strategy` inferred; `complete(id, fields, reason)`: a draft's frontmatter in one step. Each takes a reason and writes in one order: validated by loading the catalog with the new text, then written and logged; the tools re-mirror it. |
...
| `serve.py` | `/board`, `/infer`, `/evaluate`, `/strategies`, `/health` - the same functions, over HTTP, for a board that runs in another container. `/board` solves no countered case, and a newer board from the same `client` supersedes one still solving. |
