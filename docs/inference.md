...
| `engine.py` | `infer` (the optimal six around the locked picks), `evaluate` (a full six ranked against the field), `current` (the picks as they stand, partial or full), and `board` (blue's absolute optimal - no locks - and red's around its revealed picks, on opposite sides, plus the current comp scored on blue's optimal's scale). Each result carries the picks with reasons and `[F#]` citations into the board's FactSet, the score breakdown per strategy, alternatives, and the prose constraints as "ground rules to reconcile against". |
...
<!-- generated:catalog -->
38 files in `inference/strategies/`: 13 constraints (3 limits, 10 scored), 20 heuristics and 5 assumptions. Regenerated by `python -m db.mcp call db_docs`.
...
##### Locked picks are given, not chosen (`locked-picks`, assumptions)
