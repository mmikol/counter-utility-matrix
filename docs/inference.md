...
<!-- generated:catalog -->
300 files in `inference/strategies/`: 56 constraints (1 limits, 55 scored), 244 heuristics and 0 assumptions. Regenerated by `python -m db.mcp call db_docs`.
...
weight 1; when `enemy.hps_floor <= params.THIN_HEAL and enemy.supports >= 2`; bonus `min(team.dps_floor / params.PRESS_FLOOR, 2) * 0.5`
params: PRESS_FLOOR=500, THIN_HEAL=320
...
params: GOOD_WIN=50, POPULAR_MASS=45
...
weight 1; when `enemy.size >= 1 and enemy.range_median <= 30`; bonus `min(team.cc_count, params.CC_CAP) * 0.5`
params: CC_CAP=3
...
weight 1; when `team.style_lean == 'dive' and enemy.invuln + enemy.cleanse >= params.SAVES`; penalty `min(enemy.invuln + enemy.cleanse, 4) * 0.5`
params: SAVES=2
...
weight 1; when `team.availability < params.COINFLIP`; penalty `1.5`
params: COINFLIP=0.5
...
##### Four supports make no pressure (`four-supports-no-pressure`, shape, scored)

weight 1; when `team.supports >= 4`; penalty `1.5`

Two tanks and four supports is too much sustain and not enough burst to stop anyone pushing. Supports deal damage but cannot focus it or sustain it the way a damage kit does, and without the threat of damage picks the enemy holds no fear of stepping into the open, so the tanks get no space to take. It charges a flat point and a half once four supports are on the six.
...
weight 1; when `team.supports >= 3 and team.heal_ratio >= params.OVERHEAL`; penalty `0.75`
params: OVERHEAL=1.5
...
##### Three damage need both tanks (`three-damage-both-tanks`, shape, scored)

weight 1; when `team.damage >= 3 and team.tanks < 2`; penalty `1.5`

Three damage picks are carried by two tanks or not at all. A solo tank in front of a damage-heavy six has to survive, protect the backline and take ground without an off-tank's cover, and a damage line with no front to shoot from is three flankers hoping for picks. It charges a flat point and a half once three or more damage picks sit in front of fewer than two tanks.
...
##### Build around the strong pick (`build-around-strong-pick`, synergy, scored)

weight 1; when `team.win_mean >= params.STRONG`; bonus `min(team.synergy_edges, params.PAIR_CAP) * 0.5`
params: PAIR_CAP=4, STRONG=50
...
##### Two rank swingers, no rate (`two-swingers-no-rate`, uncertainty, scored)

weight 1; when `team.rank_sensitive_count >= params.SWINGERS`; penalty `1`
params: SWINGERS=2
...
##### Beams melt a fat comp (`beams-melt-fat-comps`, damage)

`maximize team.beam` - picks with a beam. weight 1; when `enemy.squish_count <= 2 and enemy.size >= 5`
...
##### Boost a hitscan pair (`boost-the-hitscan`, damage)

`maximize team.dmg_amp` - picks that amplify someone's damage. weight 1; when `team.hitscan >= 2`

Damage amplification is worth most on picks that land every shot, so a six with two hitscan guns has the right targets for a Mercy beam, a Nano or a Discord. A pocketed hitscan takes over games because a 170-damage boosted shot or a 190 dynamite crosses the one-shot line on a 225 pool, and the pocket is what puts it there. Measured as the count of picks that amplify someone's damage, read while the six carries 2 or more hitscan picks.

##### Brawl maps stack win conditions (`brawl-maps-stack-ults`, damage)

`maximize team.dmg_ults` - ultimates that carry a damage figure. weight 0.75; when `map.style_top == 'brawl'`
...
`maximize team.aoe_count` - kit pieces tagged area of effect. weight 0.75; when `team.style_lean == 'brawl'`

A brawl six fights where both teams stand within arm's reach, and area damage hits everything in that press without aiming at any of it. Reaper's shotguns and Reinhardt's hammer pay off most when the two balls collide. Measured as kit pieces tagged area of effect, read only while brawl is the majority style.
...
`maximize team.ult_damage_total` - summed max damage across the team's damage ultimates. weight 1; when `enemy.barrier_count >= 2`
...
`maximize team.burst_max` - the biggest single damage figure on the team. weight 1; when `team.style_lean == 'dive'`
...
##### Extra supports carry guns (`extra-supports-carry-guns`, damage)

`maximize team.dps_count` - picks whose kit publishes a per-second damage figure. weight 1; when `team.supports >= 3`
...
`maximize team.dps_floor` - summed published per-second damage figures (a floor: misses and healing ignored). weight 0.75; when `matchup.style_lean_red == 'brawl'`
...
`maximize team.hitscan` - picks with a hitscan weapon or ability. weight 1; when `matchup.style_lean_red == 'brawl'`
...
`maximize team.range_max` - the longest range on the team. weight 0.5
...
`maximize team.dmg_amp` - picks that amplify someone's damage. weight 0.75; when `team.style_lean == 'poke'`
...
`maximize team.hitscan` - picks with a hitscan weapon or ability. weight 1; when `team.style_lean == 'poke'`
...
##### Armor blunts a heavy floor (`armor-blunts-heavy-floor`, durability)

`maximize team.armor_total` - summed armor. weight 1; when `enemy.dps_floor >= 900`
...
`maximize team.armor_total` - summed armor. weight 1; when `enemy.hitscan >= 3`
...
`maximize team.barrier_count` - picks with a barrier. weight 1; when `enemy.burst_max >= 500`

When red carries a single hit of 500 or more, a Self-Destruct, a RIP-Tire or an Orisa spear, no heal arrives in time and the only answer is something that takes the hit instead of a player. The best counter to a Reinhardt ultimate is a Reinhardt barrier, and shields to block a D.Va bomb are what low ranks ask their tank for. Measured as our picks with a barrier, read while red's biggest single hit is 500 or more.

##### Barriers against a sniper (`barriers-against-snipers`, durability)

`maximize team.barrier_count` - picks with a barrier. weight 1; when `enemy.range_max >= 60`
...
`minimize team.squish_count` - picks at or under 250 pool. weight 1; when `enemy.dmg_amp >= 2`

A red with two damage amplifiers, a Mercy beam, a Discord, a Nano Boost or Baptiste's window, turns a 170 damage shot into a kill on any 225 pick in two hits, and nothing heals a headshot. Against that line the comp wants as few picks at 250 or under as the shape allows. Measured as our picks at or under 250 pool, kept low while 2 or more red picks amplify damage.

##### Bigger ball wins the brawl (`brawl-mirror-pool`, durability)

`maximize matchup.pool_diff` - blue effective HP minus red. weight 1; when `team.style_lean == 'brawl' and matchup.style_lean_red == 'brawl'`
...
`maximize team.pool_min` - the weakest pick's pool - focus fire finds the minimum. weight 1; when `team.style_lean == 'brawl'`
...
`maximize team.armor_total` - summed armor. weight 1; when `team.style_lean == 'brawl'`
...
`maximize team.pool_min` - the weakest pick's pool - focus fire finds the minimum. weight 0.75; when `map.stages >= 3`
...
##### One-shots hunt squishies (`one-shots-hunt-squishies`, durability)

`minimize team.squish_count` - picks at or under 250 pool. weight 1.5; when `enemy.burst_max >= 250`
...
##### Outpool the enemy (`outpool-the-enemy`, durability)

`maximize matchup.pool_diff` - blue effective HP minus red. weight 1; when `enemy.size >= 4`
...
`maximize team.shield_total` - summed recharging shields. weight 0.5
...
##### Shields shrug off chip (`shields-shrug-off-chip`, durability)

`maximize team.shield_total` - summed recharging shields. weight 0.75; when `enemy.size >= 1 and enemy.burst_max < 400`
...
`maximize team.armor_total` - summed armor. weight 1; when `team.tanks <= 1`
...
##### Counter-pick on this map only (`answers-that-fit-map`, map)
...
`maximize team.range_max` - the longest range on the team. weight 0.75; when `map.mode == 'Escort'`
...
`minimize team.melee` - picks with a melee weapon. weight 1; when `map.style_top == 'dive'`
...
##### Map affinity outlasts the patch (`map-affinity-outlasts-rates`, map)

`maximize team.map_specialists` - picks running 2.5+ points over their own baseline here. weight 1; when `map.known == 1 and team.win_mean < 50`
...
##### The map outweighs the counter (`map-outweighs-the-counter`, map)

`maximize team.map_win_mean` - mean win rate on the map (the all-ranks mean without a map). weight 1; when `map.known == 1 and team.exposed_count >= 1`
...
##### Answer the ones who answer you (`answer-the-answerers`, matchup)

`maximize team.answer_edges` - (enemy, pick) counter edges: picks answering enemies. weight 0.75; when `team.exposed_count >= 1`
...
##### Anti-heal melts a tank pair (`antiheal-melts-two-tanks`, matchup)

`maximize team.antiheal` - picks with anti-heal. weight 1.25; when `enemy.tanks >= 2`

Two tanks on red are 900 to 1200 hit points that only die with their healing switched off, and anti-heal is the tool the community names first for every heavy tank from Mauga to Reinhardt. A grenade on a tank at half health turns a sustained trade into a kill, which is why Ana's grenade is listed beside Defense Matrix as the strongest ability against Mauga. Measured as the count of picks with anti-heal, read while red fields 2 tanks.

##### Anti-heal turns off self-sustain (`antiheal-stops-self-sustain`, matchup)

`maximize team.antiheal` - picks with anti-heal. weight 1; when `enemy.lifelines >= 4`
...
##### Armor eats rapid fire (`armor-eats-rapid-fire`, matchup)

`maximize team.armor_share` - armor / pool. weight 1; when `enemy.hitscan >= 3`
...
`maximize team.beam` - picks with a beam. weight 1; when `matchup.style_lean_red == 'dive'`
...
##### Beams melt on armor (`beams-melt-on-armor`, matchup)

`minimize team.beam` - picks with a beam. weight 0.5; when `enemy.armor_total >= 300`
...
`maximize team.cc_count` - picks with crowd control (stun, sleep, immobilize, hinder, knockback). weight 1; when `team.style_lean == 'brawl'`
...
`maximize team.cleanse` - picks with a cleanse. weight 1; when `enemy.cc_count >= 3`

A red team stacked with stuns, sleeps, hinders and knockbacks wins by chaining them onto one target, and a cleanse breaks the chain before the follow-up lands. Kiriko is the community's counterpick into a team with a lot of crowd control, and a second cleanse on the six means the second chain fails too. Measured as the count of picks with a cleanse, read while 3 or more red picks carry crowd control.

##### Cleanse the counter's tool (`cleanse-the-counters-tool`, matchup)

`maximize team.cleanse` - picks with a cleanse. weight 0.75; when `team.exposure_edges >= 3`
...
`maximize team.deployables` - picks with deployables. weight 0.75; when `matchup.dive_pressure >= 4`
...
`maximize team.dmg_amp` - picks that amplify someone's damage. weight 1; when `enemy.armor_total >= 300`
...
##### Countered but ahead on the ledger (`exposed-but-ahead`, matchup)

`maximize matchup.net_edges` - blue answer edges minus blue exposure edges. weight 0.75; when `team.exposed_count >= 1`
...
##### Fly over a projectile team (`fly-over-projectile-spam`, matchup)

`maximize team.flyers` - picks that fly or hover. weight 1; when `enemy.projectile >= 4`

A red team whose guns are mostly projectiles cannot lead a target in the air, and a flier duels projectile-only heroes at a large advantage. Pharah is named as one of the best picks into Torbjörn because he is not hitscan, and a Junkrat and Mei pair into an air pick is told it will be farmed. Measured as the count of picks that fly or hover, read while 4 or more red picks carry projectile weapons.
...
`maximize team.range_median` - median of each pick's longest published range. weight 1; when `enemy.range_max <= 60 and enemy.size >= 5`
...
##### Loners lose the ledger (`loners-lose-the-ledger`, matchup)

`maximize team.net_edges` - answer edges minus exposure edges. weight 0.75; when `enemy.isolated_count >= 2`
...
##### Outreach their brawler (`outreach-their-brawler`, matchup)

`maximize team.range_median` - median of each pick's longest published range. weight 1; when `enemy.range_min <= 10 and enemy.size >= 1`
...
##### Reach the flier (`reach-the-flier`, matchup)

`maximize team.range_max` - the longest range on the team. weight 1; when `matchup.flyers >= 1`

When red fields a pick that flies, the six needs a gun whose published range reaches the air, because a flier picks the distance of every exchange. Cassidy and Ashe apply excellent pressure on Mercy, Pharah and Echo through damage projection at range, and a Pharah blows up a Torbjörn's turrets from a distance he cannot answer. The longest published range on the six is measured, read while red fields at least one flier.

##### Red overcommits on one pick (`red-overcommits-on-one`, matchup)

`maximize team.safe_count` - picks no enemy answers. weight 0.75; when `team.exposure_edges >= 3`
...
`maximize team.win_mean` - mean all-ranks win rate. weight 1; when `enemy.size >= 4`
...
##### A tank answer needs a second (`tank-answer-needs-second`, matchup)

`maximize matchup.double_covered` - red picks answered twice over. weight 0.75; when `enemy.tanks >= 2`

When red fields two tanks, a single answer to either is a duel the answering pick may lose, and the six should hold two answers for its tank matchups. Zarya into Mauga farms free charge but needs support help to not get overrun once she is out of bubbles, and a tank with no hard counter needs teamwork with the damage heroes to shut down. Measured as the count of revealed red picks answered by two or more of ours, while red fields two tanks.
...
##### Two answers to the must-ban (`double-the-must-ban`, meta)

`maximize matchup.double_covered` - red picks answered twice over. weight 0.75; when `enemy.max_ban_rate >= 30`

A hero the lobby usually bans is a hero one answer does not solve, so when one stands on red the six wants two picks that answer it. Mauga, the ban of choice for many tank players, has two answers on almost every map, and one answer that a ban or a swap removes leaves nothing. Measured as the count of revealed red picks answered by two or more of ours, while red's highest ban rate is 30 percent or more.

##### A banned answer needs a backup (`magnet-needs-backup-answer`, meta)

`maximize team.banproof_coverage` - coverage recomputed without the highest-ban answerer. weight 1; when `team.max_ban_rate >= 15`
...
`minimize team.range_max` - the longest range on the team. weight 0.5; when `team.style_lean == 'brawl'`
...
`maximize team.flyers` - picks that fly or hover. weight 0.75; when `team.style_lean == 'dive'`
...
##### Match a two-tank front (`match-two-tank-fronts`, shape)

`maximize team.tanks` - tank count. weight 1; when `enemy.tanks >= 2`
...
`maximize team.cc_count` - picks with crowd control (stun, sleep, immobilize, hinder, knockback). weight 1; when `team.tanks <= 1`
...
`maximize team.subrole_diversity` - distinct subroles / size (1.0 = every pick a different job). weight 0.5; when `team.supports >= 3`
...
##### A main tank and an off-tank (`two-tanks-two-jobs`, shape)

`maximize team.subrole_diversity` - distinct subroles / size (1.0 = every pick a different job). weight 1; when `team.tanks >= 2`

A tank pair wants a main tank that sets the fight and an off-tank that covers the angle, not two of the same. The main tank establishes how and when the fight happens while the off-tank manages the high ground and peels, and a pair of two off-tanks has nobody to walk behind. Distinct subroles divided by picks across the six is read, only while two tanks are on the six, so two tanks of one job lower it.
...
`maximize team.ult_damage_total` - summed max damage across the team's damage ultimates. weight 1; when `map.side == 'attack'`
...
`minimize matchup.chew_time_ours` - seconds of blue's floor damage to chew red's pool (999 if unknown). weight 1; when `map.side == 'attack' and matchup.chew_time_ours < 999`
...
##### Attackers pick the counters (`attackers-pick-counters`, side)

`maximize matchup.coverage_share` - share of red answered by blue. weight 0.75; when `map.side == 'attack' and enemy.size >= 1`
...
`maximize matchup.ult_answers` - blue invulnerabilities plus cleanses. weight 1; when `enemy.ult_cost_mean <= 2100 and enemy.size >= 3`

A red whose ultimates cost 2,100 charge or less on average fires them a fight sooner than a comp of expensive ones, so the invulnerabilities and cleanses that eat an ultimate get used every fight rather than every other. Kiriko negating half the roster's ultimates is the community's example of what that answer count is worth. Measured as our invulnerabilities plus cleanses, read while 3 or more red picks are revealed and their mean ultimate cost is 2,100 or less.

##### Cleanse a loaded ultimate bar (`cleanse-loaded-ult-bars`, sustain)

`maximize team.cleanse` - picks with a cleanse. weight 0.75; when `matchup.ult_threat >= 1000`

When red's damage ultimates add up to 1,000 or more, the fight that matters is the one where two of them land together, and a cleanse is the button that lifts a Graviton's hold or an anti-heal in the middle of it. Kiriko's Suzu is credited with negating half the roster's ultimates for exactly that reason. Measured as our picks with a cleanse, read while red's summed ultimate damage is 1,000 or more.

##### Control rewards area healing (`control-area-healing`, sustain)
...
`maximize team.lifelines` - picks carrying any healing at all. weight 1; when `team.hps_floor < world.heal_bench`
...
##### Seat one real healer (`one-real-healer`, sustain)

`maximize team.heal_ratio` - support heal peak / the roster's two-support bench. weight 0.75

A support line heals at the roster's bench or the six is not sustained at all. One main healer with high burst healing is the pairing the game rewards, and a Lúcio, Brigitte or Mizuki alone cannot be relied on as the main healer because the single-target heal is never enough. Measured as the supports' summed heal peak over the roster's two-support bench.
...
##### Big saves against heavy damage (`saves-against-heavy-damage`, sustain)

`maximize team.heal_ratio` - support heal peak / the roster's two-support bench. weight 1; when `enemy.dps_floor >= 950`
...
##### Saves against loaded ultimates (`saves-against-loaded-ults`, sustain)

`maximize matchup.heal_vs_burst` - blue's biggest single save minus red's biggest hit. weight 1; when `enemy.dmg_ults >= 4`

When red carries four or more damage ultimates, the biggest single save on our side is what turns a wipe into a fight. Sound Barrier protecting from D.Va's bomb and burst damage in general is what kept GOATS alive, and a Biotic Grenade that brings four teammates back from critical is the play that beats a combo. Measured as our biggest single heal minus red's biggest hit, read while red fields four or more ultimates that carry a damage figure.
...
`maximize team.heal_peak_total` - summed peak single heal across all picks, any role. weight 0.5

Every kit that heals adds effective hit points that no enemy sees on the bar. Baptiste's regen burst makes him 350 effective HP in a duel, and a Moira's fade and drain add effective HP on top of her pool, so two sixes with the same summed pool are not equally hard to kill. Summed peak single heal across all picks, any role, is measured.
...
A dive lands on the supports first, and the picks that heal themselves are the ones still fighting while the supports are busy living. The low-rank situations guide's answer to a team that cannot be healed is picks that are more self-sufficient, and a D.Mon player explains adapting to a low-healing backline by playing health packs and angles. Measured as the count of picks carrying any healing at all, read while 4 or more red picks carry a movement tool.

##### A solo healer needs lifelines (`solo-heal-lifelines`, sustain)

`maximize team.lifelines` - picks carrying any healing at all. weight 0.75; when `team.supports <= 1`
...
##### Self-heals cover a thin line (`thin-line-self-heals`, sustain)

`maximize team.heal_peak_total` - summed peak single heal across all picks, any role. weight 1; when `team.supports <= 1`
...
`maximize team.core_size` - largest connected group in the team's synergy graph. weight 1; when `team.style_lean == 'dive'`
...
`maximize team.synergy_density` - synergy edges / possible pairs. weight 0.75; when `team.exposed_count >= 2`
...
`minimize team.isolated_count` - picks with no authored partner on the team. weight 0.75; when `matchup.dive_pressure >= 4`
...
`maximize team.synergy_score` - summed synergy scores among the picks. weight 1; when `enemy.size >= 1 and team.coverage < enemy.size`
...
`minimize team.ult_cost_mean` - mean ultimate charge cost where published. weight 0.5; when `map.side == 'attack'`
...
`minimize team.cooldown_median` - median cooldown across every ability on the team. weight 1; when `team.style_lean == 'dive'`
...
##### More cooldowns to trade (`more-cooldowns-to-trade`, tempo)

`maximize team.cooldown_count` - cooldowns counted. weight 0.5; when `enemy.cooldown_count >= 15`
...
`minimize team.ult_cost_mean` - mean ultimate charge cost where published. weight 0.75; when `enemy.tanks >= 2`
