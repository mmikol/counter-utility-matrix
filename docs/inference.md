...
| `plan.py` | The board in prose: `momentum`, the verdict read off the two current comps (each seat's share, the fight odds, blue against red's best counter), and `plan`, the game plan - the ground and its terrain, the style to play, what red's picks mean and which picks of the six answer them, the family to stay in, what the six is built for, and what it rests on - read off the same facts and strategies the solver scored. |
