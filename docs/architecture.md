...
| `tests/` | one folder per layer: `tests/db`, `tests/ui`, `tests/inference`, and `tests/fixtures/playbook/` - the former 38-rule playbook, kept as the reference every kind and form of strategy is proven against while `inference/strategies/` holds the user's own rules; `pytest -q` runs them, skipping what needs a built database when there is none | |
