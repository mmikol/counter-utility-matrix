...
| `schema.py` | A tool as the protocol serves it: its arguments as JSON Schema (`ToolSchema`, which `tool_schema` builds, a `Property` per argument), its reply (`ToolReply`: text, and the same as JSON), and the `Tool` that checks every call against the schema before the tool runs. |
