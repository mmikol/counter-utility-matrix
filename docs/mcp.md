...
<!-- generated:tools -->
33 tools, in the order the server lists them. Regenerated by `python -m db.mcp call db_docs`.
...
| `load_authored` | Store the one input a user writes: the mirror of the strategies in inference/strategies/. A whole-truth reload. | `only` (array): accepted for older callers; strategies is the only input |
