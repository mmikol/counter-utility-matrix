...
| `registry.py` | A tool as registered: `ToolSpec` (name, description, JSON schema, function, and for a pull the source it reads). `Registry` holds a family's tools in registration order, refuses a name twice and derives the pulls; `joined` assembles the families, `run` is the audited in-process call, `write_docs` the tool reference in [mcp.md](mcp.md). `Context` is where a call lands - the database, the page caches, the log - and carries the joined registry, through which one tool calls another. |
