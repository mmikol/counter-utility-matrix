...
| `data/fetch.py` | `cached_get`: one page, from the cache if it is there and fresh. `cached`: the cache sequence every source reads through - the fresh copy, else a new one written, else the stale copy. `request`: one page under a `RequestPolicy` - its attempts, backoff, timeout and the pause after a page - retried while attempts remain. `session`: a requests session that says who we are. `PullContext`: what a pull's `run()` takes beside its connection and every fetch helper takes whole - the page cache, the session, the log (stderr unless the caller names another, since over stdio stdout is the MCP wire) and `max_age`, the freshness: a build keeps every cached page, a refresh refetches them, and a page that fails to refetch keeps its cached copy and is named in the context's `stale`. `prepare_cache`: the cache directory a tool hands a pull. |
...
| file | purpose |
| --- | --- |
| `__init__.py` | Where the database is: `default_dsn` resolves `DATABASE_URL`, or the embedded cluster at `db/psql/cluster` once one is built, and never creates one; `boot`, which only `db_init` and `db_rebuild` call, creates it; with neither, `NoDatabaseError` (a host without pgserver must set `DATABASE_URL`); how a source registers the `sources` row its rows carry; `lookup_ids`, a column's values to their ids as stored (a code is read as it is, a source's name goes through `data/names.py`); what a capture is stamped with (now, the current patch and season); the CSV export and its `EXPORT.json` mark naming the database it came from. Knows no particular source or table. |
